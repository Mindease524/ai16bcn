...
import { ForecastMarket } from './forecast_market';
...
export class ForecastMarketProgram {
  ...
  public bettingPDA(voter: PublicKey, marketKey: BN, anwserKey: BN): PublicKey {
    return PublicKey.findProgramAddressSync(
      [
        Buffer.from("betting"),
        voter.toBuffer(),
        marketKey.toBuffer("le", 8),
        anwserKey.toBuffer("le", 8),
      ],
      this.program.programId
    )[0];
  }
  ...
  public async retrieveTokens(
    ...
    const remainTokenAccount = await getAssociatedTokenAddress(
      marketData.betMint,
      configData.remainAccount,
      true
    );
  ...
  public async claimToken(
    ...
    const userRewardTokenAccount = await getAssociatedTokenAddress(
      configData.rewardMint,
      voter
    );
    const vaultBetTokenAccount = await getAssociatedTokenAddress(
      marketData.betMint,
      marketAccount,
      true
    );
    const vaultRewardTokenAccount = await getAssociatedTokenAddress(
      configData.rewardMint,
      configAccount,
      true
    );
   ...
   */
  public async getPendingRewardData(
    voter: PublicKey,
    marketKey: BN,
    anwserKey: BN
  ): Promise<{ recieveTokens: BN; pendingReward: BN }> {
    ...
    if (
      ...
      marketData.correctAnswerKey.toString() == anwserKey.toString()
    ) {
      ...
      for (let i = 0; i < answerData.answers.length; i++) {
        if (
          answerData.answers[i].answerKey.toString() ==
          marketData.correctAnswerKey.toString()
        ) {
      ...
      percentage = marketRewardBaseTokens
        .mul(new BN(10000))
    ...
    const pendingReward = bettingData.tokens
      .mul(rewardAPR)
      .div(new BN(10000))
      .mul(timeStaked)
      .div(new BN(31536000));
  ...
  public async getMarketVaultBalance(marketKey: BN): Promise<any> {
    ...
    const marketVaultBalance = await this.connection.getTokenAccountBalance(
      vaultTokenAccount,
      "confirmed"
    );
